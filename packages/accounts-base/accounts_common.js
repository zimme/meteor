AccountsCommon = function AccountsCommon(ddpUrl) {
  // Currently this is read directly by packages like accounts-password
  // and accounts-ui-unstyled.
  this._options = {};

  this._initConnection(ddpUrl);

  this._users = new Mongo.Collection("users", {
...
Ap.user = function () {
  ...
  return userId ? this._users.findOne(userId) : null;
};
...
Ap._initConnection = function (url) {
  if (! Meteor.isClient) {
    return;
  }
...
Ap.getTokenLifetimeMs = function () {
...
Ap._tokenExpiresSoon = function (when) {
  var minLifetimeMs = .1 * this.getTokenLifetimeMs();
