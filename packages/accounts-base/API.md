...
Before authenticating the user, a login service generates a secure GUID to use as a "login ID". It passes it to `Accounts.addLoginId(loginId)` on the client and, if necessary, also passes it as part of the "state" that might end up in a separate client if the authentication process ends in a different browser than the one where it started. In the (potentially different) client, the login service finishes the authentication process, stores the "login ID" in a new user object that is marked as "unregistered" (to prevent login), and passes the login ID to `Accounts.addLoginId(loginId)`. User can then use either client to register gathering any additional registration information from the user, asking/confirming which logins to link to the new account, and passing the login IDs into `Accounts.register` along with the other information required to register. The server uses the login IDs to finds the existing unregistered user(s) and add the registration information to it or make it/them alias for a new registered user. All of this means that the user doesn't need to authenticate a second time when `Accounts.register` is called.
...
```js
// Register a function to be called when continuing a Sign-Up with Service X action in a new client.
...
Accounts.onContinue(Accounts.SIGN_UP, (err) => {
  ...
  /* 
    Accounts.getLogins() returns an array of logins that the current end-user has authenticated with:
    [
      {
  ...
  */
  // Gather registration information from the logins and any additional registration information from the 
  // end-user and pass it to Accounts.register()
  UI.gatherRegistrationInfo(logins, (regData) => {
    // regData.loginIds should contains the login IDs for the logins that can be used to sign-in to the
    // new account.
    Accounts.register(regData, (err) => {
      ...
      // User was successfully signed up, is now logged in, and can login in the future with logins
    });
  });
});
