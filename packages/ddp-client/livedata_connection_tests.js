...
if (Meteor.isClient) {
  Tinytest.add("livedata stub - methods", function (test) {
    ...
    stream.receive({msg: 'added', collection: collName, id: MongoId._idStringify(docId),
...
if (Meteor.isClient) {
  Tinytest.add("livedata stub - methods calling methods", function (test) {
    ...
    stream.receive({msg: 'added', collection: coll_name, id: MongoId._idStringify(docId),
                    fields: {value: 'tuesday'}});
    o.expectCallbacks();
...
if (Meteor.isClient) {
  Tinytest.add("livedata stub - reconnect method which only got result", function (test) {
    ...
    // Get some data.
    stream.receive({msg: 'added', collection: collName,
                    id: MongoId._idStringify(stubWrittenId), fields: {baz: 42}});
    ...
    // Get some data.
    stream.receive({msg: 'added', collection: collName,
                    id: MongoId._idStringify(stubWrittenId2), fields: {baz: 42}});
    // It doesn't show up yet.
    ...
    stream.receive({msg: 'added', collection: collName,
                    id: MongoId._idStringify(stubWrittenId2), fields: {foo: 'bar'}});
    o.expectCallbacks();
...
if (Meteor.isClient) {
  Tinytest.add("livedata stub - multiple stubs same doc", function (test) {
    ...
    stream.receive({msg: 'added', collection: collName,
                    id: MongoId._idStringify(stubWrittenId), fields: {foo: 'barb', other: 'field',
                                                                    other2: 'bla'}});
    // It doesn't show up yet.
    test.equal(coll.find().count(), 1);
    test.equal(coll.findOne(stubWrittenId),
    ...
    stream.receive({msg: 'changed', collection: collName,
                    id: MongoId._idStringify(stubWrittenId), fields: {baz: 43}, cleared: ['other']});
    test.equal(coll.find().count(), 1);
    test.equal(coll.findOne(stubWrittenId),
               {_id: stubWrittenId, foo: 'bar', baz: 42});
    o.expectCallbacks();
